...
public class ICollectionDaoImpl implements ICollectionDaoCustom {
	private static final List<String> NumberFields=Arrays.asList("collectionId","personId","videoId");
	private static final List<String> StringFields=Arrays.asList("userName","videoTitle","createTime");
	@PersistenceContext
	private EntityManager em;
	@Override
	public Page<CollectionEntity> findAllWithFilters(Pageable pr,
			ExtFilters filters) {
		String from="from CollectionEntity ";
		...
		if(total>0){
            ...
            if(pr.getSort()!=null){
                ...
                order=new Sort.Order(Sort.Direction.DESC, "collectionId");
            }

		 	List<CollectionEntity> data=em.createNativeQuery("select * "+from+" order by "+order.getProperty()+" "+order.getDirection().name(), CollectionEntity.class).setFirstResult(pr.getOffset()).setMaxResults(pr.getPageSize()).getResultList();
		 	return new PageImpl<CollectionEntity>(data, pr, total);
		}else{
			return new PageImpl<CollectionEntity>(Collections.EMPTY_LIST, pr, total);
		}
	}
